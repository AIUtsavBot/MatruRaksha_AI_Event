...
import { FileText, Upload, Loader, ExternalLink, AlertCircle, CheckCircle, Trash2, Eye, RefreshCw } from 'lucide-react'

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000'

export default function DocumentManager({ motherId, motherName }) {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            // First, we need to upload to storage and get the URL
            // For now, we'll create a placeholder document entry
            // In production, this would upload to Supabase Storage first

            const fileType = file.type.split('/')[1] || 'unknown'

            // Create a temporary URL for the file (in production, this would be the Supabase Storage URL)
            ...
            reader.onloadend = async () => {
                try {
                    ...
                    const payload = {
                        ...
                        mother_id: motherId,
                        file_url: reader.result.substring(0, 100) + '...', // Placeholder
                        file_type: fileType
                    }

                    // Note: In a real implementation, you'd upload to Supabase Storage first
                    // and then call the analyze-report endpoint with the actual file URL
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${getStatusColor(doc.analysis_status)}`}>
                                        ...
                                        <div className="text-sm text-gray-600 space-y-1">
                                            ...
                                            <p>📄 Type: {doc.file_type?.toUpperCase() || 'Unknown'}</p>

                                            {doc.analysis_result?.risk_level && (
                                                <p className="flex items-center gap-1">
                                                    ⚠️ Risk:
                                                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${getRiskColor(doc.analysis_result.risk_level)}`}>
                                                        {doc.analysis_result.risk_level}
                                                    </span>
                                                </p>
                                            )}
                                            ...
                                            {doc.analysis_result?.concerns?.length > 0 && (
                                                <div className="mt-2 p-2 bg-yellow-50 rounded">
                                                    <p className="text-xs font-semibold text-yellow-800 mb-1">Concerns:</p>
                                                    <ul className="text-xs text-yellow-700 list-disc pl-4">
                                                        {doc.analysis_result.concerns.slice(0, 3).map((c, i) => (
                                                            <li key={i}>{c}</li>
                                                        ))}
                                                    </ul>
                                                </div>
