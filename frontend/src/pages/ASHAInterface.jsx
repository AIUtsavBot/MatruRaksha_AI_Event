...
import {
  Users, Search, Phone, MapPin, AlertCircle, Heart, TrendingUp, Loader, RefreshCw,
  Activity, FileText, ClipboardCheck, BarChart2, ChevronLeft, Calendar, Thermometer, UserPlus
} from 'lucide-react'
...
export default function ASHAInterface() {
  ...
  const [loadingMotherAssessments, setLoadingMotherAssessments] = useState(false)
  ...
  useEffect(() => {
    const autoDetectAsha = async () => {
      ...
      try {
        ...
        setError('Your account is not linked to an ASHA worker profile. Contact admin.')
      } catch (err) {
  ...
  const handleAssessRisk = async (e) => {
    ...
    try {
      ...
      const telegramMsg = response.telegram_sent ? '📱 Report sent to mother!' : ''
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">Loading ASHA Portal</h1>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 mt-2">
              {user?.email ? `No ASHA worker found for: ${user.email}` : 'Please login first'}
            </p>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
            <p className="text-yellow-800 text-sm font-semibold mb-2">📋 To fix this:</p>
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-80 bg-white border-r flex flex-col shadow-lg">
        ...
        <div className="flex border-b">
          <button
            onClick={() => { setSelected(null); setMainView('register') }}
            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1 ${mainView === 'register' ? 'text-pink-600 bg-pink-50' : 'text-gray-500'
              }`}
          >
          ...
          <button
            onClick={() => { setSelected(null); setMainView('assess') }}
            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1 ${mainView === 'assess' ? 'text-green-600 bg-green-50' : 'text-gray-500'
              }`}
          >
          ...
          <button
            onClick={() => { setSelected(null); setMainView('stats') }}
            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1 ${mainView === 'stats' ? 'text-green-600 bg-green-50' : 'text-gray-500'
              }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold text-gray-600 uppercase">Mothers ({filtered.length})</h2>
            <button onClick={loadMothers} className="text-green-600 hover:text-green-700">
          ...
          {loading && mothers.length === 0 ? (
            ...
            <div className="space-y-2">
              {filtered.map(m => (
                <div
                  key={m.id}
                  onClick={() => { setSelected(m); setMainView('mother') }}
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${selected?.id === m.id ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'
                    }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold text-gray-900">{m.name}</div>
                    <span className="text-lg">{getRiskIcon(riskMap[m.id] || 'LOW')}</span>
                  </div>
          ...
          <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 mt-1 text-sm flex items-center gap-2">
                    <MapPin className="w-4 h-4" /> {selected.location} · Age {selected.age}
                  </p>
                </div>
                <div className={`px-4 py-2 rounded-lg font-semibold flex items-center gap-2 ${riskMap[selected.id] === 'HIGH' ? 'bg-red-100 text-red-700' :
                  riskMap[selected.id] === 'MODERATE' ? 'bg-yellow-100 text-yellow-700' :
                    'bg-green-100 text-green-700'
                  }`}>
                  <span className="text-xl">{getRiskIcon(riskMap[selected.id])}</span>
            ...
            <div className="flex-1 overflow-hidden flex gap-6 p-6">
              ...
              <div className="w-72 bg-white rounded-xl shadow border p-5 overflow-y-auto">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="font-semibold">{selected.bmi?.toFixed(1)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-semibold capitalize">{selected.preferred_language}</p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={() => {
                    setAssessmentForm({ ...assessmentForm, mother_id: selected.id })
                    setMainView('assess')
                  }}
              ...
              <div className="flex-1 bg-white rounded-xl shadow border overflow-hidden flex flex-col">
                ...
                <div className="bg-gradient-to-r from-green-600 to-green-700 text-white px-5 py-3 flex items-center gap-4">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-sm font-semibold ${motherViewTab === 'history' ? 'bg-white/20' : 'opacity-70 hover:opacity-100'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-sm font-semibold ${motherViewTab === 'chat' ? 'bg-white/20' : 'opacity-70 hover:opacity-100'
                      }`}
                  >
                    💬 Notes
                  </button>
                    ...
                    {loadingMotherAssessments ? (
                      ...
                      <div className="space-y-3">
                        {motherAssessments.map((a, idx) => (
                          <div
                            key={a.id || idx}
                            className={`p-4 rounded-lg border-2 ${a.risk_level === 'HIGH' ? 'bg-red-50 border-red-200' :
                              a.risk_level === 'MODERATE' ? 'bg-yellow-50 border-yellow-200' :
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <span className={`px-2 py-1 rounded-full text-xs font-bold ${a.risk_level === 'HIGH' ? 'bg-red-200 text-red-800' :
                                a.risk_level === 'MODERATE' ? 'bg-yellow-200 text-yellow-800' :
                                  'bg-green-200 text-green-800'
                                }`}>
                                {getRiskIcon(a.risk_level)} {a.risk_level} ({(a.risk_score * 100).toFixed(0)}%)
                              </span>
                            </div>
                            ...
                            <div className="grid grid-cols-4 gap-2 text-xs">
                              {a.systolic_bp && a.diastolic_bp && (
                                <div className="bg-white/60 p-2 rounded text-center">
                                  ...
                                  <p className="font-bold">{a.systolic_bp}/{a.diastolic_bp}</p>
                                </div>
                              )}
                              {a.heart_rate && (
                                <div className="bg-white/60 p-2 rounded text-center">
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-xs mt-1">Click "New Assessment" to start</p>
                      </div>
                    )}
                  </div>
                ) : (
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <form onSubmit={handleAssessRisk} className="space-y-5">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Select Mother *</label>
                    <select
                      value={assessmentForm.mother_id}
                      onChange={e => setAssessmentForm({ ...assessmentForm, mother_id: e.target.value })}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setAssessmentForm({ ...assessmentForm, systolic_bp: e.target.value })}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setAssessmentForm({ ...assessmentForm, diastolic_bp: e.target.value })}
                        className="w-full px-4 py-3 border rounded-lg"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setAssessmentForm({ ...assessmentForm, heart_rate: e.target.value })}
                        className="w-full px-4 py-3 border rounded-lg"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Blood Glucose</label>
                      <input
                        type="number"
                        placeholder="100"
                        value={assessmentForm.blood_glucose}
                        onChange={e => setAssessmentForm({ ...assessmentForm, blood_glucose: e.target.value })}
                        className="w-full px-4 py-3 border rounded-lg"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setAssessmentForm({ ...assessmentForm, hemoglobin: e.target.value })}
                      className="w-full px-4 py-3 border rounded-lg"
                    />
                  </div>

                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p className="text-sm font-semibold text-gray-700 mb-3">Clinical Symptoms</p>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                ...
                {riskResult && (
                  <div className={`mt-6 p-5 rounded-lg border-2 ${riskResult.risk_level === 'HIGH' ? 'bg-red-50 border-red-300' :
                    riskResult.risk_level === 'MODERATE' ? 'bg-yellow-50 border-yellow-300' :
                      'bg-green-50 border-green-300'
                    }`}>
                    <div className="flex items-center gap-3 mb-3">
                      <span className="text-3xl">{getRiskIcon(riskResult.risk_level)}</span>
                      <div>
                        <p className="font-bold text-lg">{riskResult.risk_level} RISK</p>
                        <p className="text-sm">Score: {(riskResult.risk_score * 100).toFixed(0)}%</p>
                      </div>
                    </div>
                    {riskResult.risk_factors?.length > 0 && (
                      <div className="mt-3">
                        ...
                        <ul className="text-sm list-disc pl-5">
                          {riskResult.risk_factors.map((f, i) => <li key={i}>{f}</li>)}
                        </ul>
                      </div>
                    )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-white p-5 rounded-xl shadow border-l-4 border-blue-500">
                      <p className="text-3xl font-bold text-blue-600">{analytics.total_mothers}</p>
                    ...
                    <div className="bg-white p-5 rounded-xl shadow border-l-4 border-purple-500">
                      <p className="text-3xl font-bold text-purple-600">{analytics.total_assessments}</p>
                    ...
                    <div className="bg-white p-5 rounded-xl shadow border-l-4 border-red-500">
                      <p className="text-3xl font-bold text-red-600">{analytics.high_risk_count}</p>
                      <p className="text-sm text-gray-600">High Risk Found</p>
                    </div>
                    <div className="bg-white p-5 rounded-xl shadow border-l-4 border-green-500">
                      <p className="text-3xl font-bold text-green-600">{analytics.low_risk_count}</p>
                    ...
                    {riskDistData.some(d => d.value > 0) && (
                      <div className="bg-white p-5 rounded-xl shadow">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                          <PieChart>
                            <Pie data={riskDistData} dataKey="value" cx="50%" cy="50%" outerRadius={70} label>
                      ...
                      {analytics.recent_assessments?.length > 0 ? (
                        <div className="space-y-2 max-h-64 overflow-y-auto">
                          {analytics.recent_assessments.map(a => (
                            <div
                              key={a.id}
                              onClick={() => { setSelectedAssessment(a); setMainView('assessment-detail') }}
                              className="p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 flex justify-between items-center"
                            >
                              <div>
                                <p className="font-semibold text-sm">{a.mother_name}</p>
                                <p className="text-xs text-gray-500">
                                  <Calendar className="w-3 h-3 inline mr-1" />
                                  {new Date(a.created_at).toLocaleDateString()}
                                </p>
                              </div>
                              <span className={`px-3 py-1 rounded-full text-xs font-bold ${a.risk_level === 'HIGH' ? 'bg-red-100 text-red-700' :
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex items-center gap-4 mb-6 pb-6 border-b">
                  <span className="text-4xl">{getRiskIcon(selectedAssessment.risk_level)}</span>
                  <div>
                    <h2 className="text-xl font-bold">{selectedAssessment.mother_name}</h2>
                ...
                <div className={`p-4 rounded-lg mb-6 ${selectedAssessment.risk_level === 'HIGH' ? 'bg-red-50' :
                  selectedAssessment.risk_level === 'MODERATE' ? 'bg-yellow-50' :
                    'bg-green-50'
                  }`}>
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-sm font-semibold text-gray-600">Risk Level</p>
                      <p className="text-2xl font-bold">{selectedAssessment.risk_level}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-gray-600">Score</p>
                      <p className="text-2xl font-bold">{(selectedAssessment.risk_score * 100).toFixed(0)}%</p>
                    </div>
                  </div>
                </div>
        ...
        {mainView === 'mother' && !selected && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-gray-600 mt-2">Choose from the list or start a new assessment</p>
            </div>
          </div>
        )}
      </div>
