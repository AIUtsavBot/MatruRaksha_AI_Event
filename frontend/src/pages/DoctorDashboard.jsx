...
import {
  Stethoscope, AlertTriangle, AlertCircle, CheckCircle, MapPin, Cake,
  Activity, Heart, TrendingUp, Search, RefreshCw, Loader, FileText,
  Calendar, Thermometer, BarChart2, MessageCircle, ClipboardList, UserPlus, Upload
} from 'lucide-react'
  ...
  useEffect(() => {
    const autoDetectDoctor = async () => {
      ...
      try {
        ...
        setError('Your account is not linked to a doctor profile. Contact admin.')
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">Loading Doctor Portal</h1>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 mt-2">
              {user?.email ? `No doctor found for: ${user.email}` : 'Please login first'}
            </p>
          </div>
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="w-96 bg-white border-r border-gray-200 flex flex-col shadow-lg">
        ...
        <div className="flex border-b border-gray-200">
          <button
            onClick={() => { setMainView('patients'); setSelected(null); }}
            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1 ${mainView === 'patients' ? 'text-blue-600 bg-blue-50' : 'text-gray-500'
              }`}
          >
          ...
          <button
            onClick={() => { setMainView('register'); setSelected(null); }}
        ...
        {successMsg && (
          <div className="mx-4 mt-3 p-2 bg-green-50 border border-green-200 rounded-lg text-green-800 text-xs">
            {successMsg}
            <button onClick={() => setSuccessMsg('')} className="float-right">×</button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200 flex items-center justify-between text-xs">
                      <span className={`px-2 py-1 rounded-full font-semibold ${risk === 'HIGH' ? 'bg-red-100 text-red-700' :
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm mt-1">Patients will appear when assigned to you</p>
            </div>
          )}
        </div>
      </div>
        ...
        {selected ? (
          <div className="flex flex-col h-full">
            ...
            <div className="bg-white border-b border-gray-200 px-8 py-6 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className={`px-5 py-3 rounded-lg font-semibold flex items-center gap-2 ${riskMap[selected.id] === 'HIGH' ? 'bg-red-100 text-red-700' :
                  riskMap[selected.id] === 'MODERATE' ? 'bg-yellow-100 text-yellow-700' :
                    'bg-green-100 text-green-700'
                  }`}>
                  <span className="text-xl">{getRiskEmoji(riskMap[selected.id])}</span>
              ...
              <div className="flex gap-3 mt-4 flex-wrap">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 ${activeTab === 'history' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 ${activeTab === 'documents' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 ${activeTab === 'chat' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700'
                    }`}
                >
                  <MessageCircle className="w-4 h-4" /> Chat with Patient
                </button>
                <button
                  onClick={() => setActiveTab('consultation')}
                  className={`px-4 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 ${activeTab === 'consultation' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700'
                    }`}
                >
              ...
              <div className="w-80 bg-white rounded-xl shadow-md border border-gray-200 p-5 overflow-y-auto">
                ...
                <div className="space-y-4 text-sm">
                  <div className="pb-3 border-b">
                    ...
                    <p className="text-gray-900 font-semibold mt-1">{selected.phone || 'N/A'}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pb-3 border-b">
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">BMI</label>
                      <p className="text-gray-900 font-semibold mt-1">{selected.bmi?.toFixed(1)}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Gravida</label>
                      <p className="text-gray-900 font-semibold mt-1">{selected.gravida}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Parity</label>
                      <p className="text-gray-900 font-semibold mt-1">{selected.parity}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Language</label>
                      <p className="text-gray-900 font-semibold mt-1 capitalize">{selected.preferred_language}</p>
                    </div>
                  </div>
                  {selected.due_date && (
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Due Date</label>
                      <p className="text-gray-900 font-semibold mt-1">{new Date(selected.due_date).toLocaleDateString()}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                {activeTab === 'history' ? (
                    ...
                    <div className="flex-1 overflow-y-auto p-5">
                      {loadingAssessments ? (
                          ...
                          {assessments.map((a, idx) => (
                            ...
                            >
                              <div className="flex justify-between items-start mb-3">
                                ...
                                <div className={`px-3 py-1 rounded-full text-sm font-bold ${a.risk_level === 'HIGH' ? 'bg-red-200 text-red-800' :
                                  a.risk_level === 'MODERATE' ? 'bg-yellow-200 text-yellow-800' :
                                    'bg-green-200 text-green-800'
                                  }`}>
                                  {getRiskEmoji(a.risk_level)} {a.risk_level} ({(a.risk_score * 100).toFixed(0)}%)
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-4 gap-3 text-sm">
                                ...
                                {a.heart_rate && (
                                  <div className="bg-white/60 p-2 rounded">
                                    ...
                                    <p className="font-bold">{a.heart_rate} bpm</p>
                                  </div>
                                )}
                                {a.blood_glucose && (
                                  <div className="bg-white/60 p-2 rounded">
                                    <p className="text-xs text-gray-500">Glucose</p>
                                    <p className="font-bold">{a.blood_glucose} mg/dL</p>
                                  </div>
                                )}
                                {a.hemoglobin && (
                                  <div className="bg-white/60 p-2 rounded">
                                    <p className="text-xs text-gray-500">Hemoglobin</p>
                                    <p className="font-bold">{a.hemoglobin} g/dL</p>
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-500 text-sm mt-1">ASHA worker will perform assessments</p>
                        </div>
                      )}
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <div className="flex-1 overflow-hidden">
                      <CaseChat motherId={selected.id} userRole={'DOCTOR'} userName={doctorInfo?.name || 'Doctor'} />
                    </div>
                  </>
                ) : (
                  <>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-gray-600 mt-2">Choose a patient to view their records and assessments</p>
            </div>
          </div>
        )}
      </div>
